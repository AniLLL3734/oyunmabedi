    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-[calc(100vh-150px)] max-w-4xl mx-auto">
            ...
            <div className="p-4 bg-dark-gray rounded-b-lg border border-t-0 border-cyber-gray/50">
                ...
                <form onSubmit={sendMessage} className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
                    <div className="relative flex-1">
                    <input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder={chatSettings.chatPaused && !isAdmin ? "Sohbet şu anda devre dışı..." : "Bir mesaj gönder..."} maxLength={MAX_CHAR_LIMIT} className={`w-full p-4 min-h-[48px] bg-space-black text-ghost-white rounded-md border border-cyber-gray/50 focus:ring-2 focus:ring-electric-purple focus:outline-none ${chatSettings.chatPaused && !isAdmin ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={chatSettings.chatPaused && !isAdmin} />
                    ...
                    <div className="flex gap-2 justify-center sm:justify-start">
                        <button type="button" onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="p-4 bg-cyber-gray/50 text-white rounded-md hover:bg-cyber-gray/70 transition-all min-w-[48px] min-h-[48px]" title="Emoji Ekle"><Smile size={24} /></button>
                        <button type="submit" disabled={!newMessage.trim() || isSending || newMessage.length > MAX_CHAR_LIMIT || (chatSettings.chatPaused && !isAdmin)} className="p-4 bg-electric-purple text-white rounded-md hover:bg-opacity-80 transition-all disabled:bg-cyber-gray/50 disabled:cursor-not-allowed min-w-[48px] min-h-[48px]"><Send size={24} /></button>
                    </div>
                </form>
