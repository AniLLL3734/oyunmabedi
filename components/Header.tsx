...
const Header: React.FC = () => {
  ...
  // DEĞİŞİKLİK: 'hasUnreadMessages' değişkeni artık gerekli olmadığı için kaldırıldı.
    ...
    <header className="sticky top-0 z-40 bg-dark-gray/70 backdrop-blur-sm border-b border-cyber-gray/50">
      <nav className="container mx-auto flex justify-between items-center p-4">
        ...
        <ul className="flex items-center space-x-4 md:space-x-6 text-lg">
          ...
          {/*
              'Mesajlar' linki ve onunla ilişkili `user` kontrolü buradan tamamen kaldırıldı.
          */}
          ...
          <li>
            <motion.div variants={navItemVariants} whileHover="hover">
              <NavLink to="/chat" style={({ isActive }) => (isActive ? activeStyle : {})}>
                Ana Sohbet Odası
              </NavLink>
            </motion.div>
          </li>
          ...
          {user && (
            <li>
              <motion.div variants={navItemVariants} whileHover="hover">
                <NavLink to="/chat-rooms" style={({ isActive }) => (isActive ? activeStyle : {})}>
                   <MessagesSquare size={18} className="inline-block mr-1"/>
                   <span className="hidden md:inline">Diğer Odalar</span>
                </NavLink>
              </motion.div>
            </li>
          )}
